...
class ReduceCashCommand extends Command
{
	...
	{
		$this->plugin = $plugin;
		parent::__construct ("캐쉬 차감", "캐쉬 차감 명령어 입니다.");
	...
	public function execute (CommandSender $player, string $label, array $args): bool
	{
		if ($player->hasPermission (self::PERMISSION)) {
			if (isset ($args [0]) and isset ($args [1]) and is_numeric ($args [1])) {
				$this->plugin->reduceCash ($args [0], $args [1]);
				SCash::message ($player, "§a{$args [0]}님§7에게 §a" . number_format ($args [1]) . "§7 만큼의 캐쉬를 차감했습니다.");
				if (($target = $this->plugin->getServer ()->getPlayer ($args [1])) !== null) {
					SCash::message ($target, "관리자 {$player->getName ()}님 께서 당신에게 §a" . number_format ($args [1]) . "§7 만큼의 캐쉬를 차감했습니다.");
				}
			} else {
				SCash::message ($player, "/캐쉬 차감 (닉네임) (캐쉬)");
			}
		} else {
